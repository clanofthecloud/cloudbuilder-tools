...
Lobby::Lobby() : verbose(true) {
	...
	commands.emplace_back("resume", "Tries to resume the last played match", " [match_id]", [&] (Tokenizer& cmd) {
		...
		CMatchManager::Instance()->HLRestoreMatch(matchId, MakeMatchResultHandler([&] (eErrorCode err, const CCloudResult *result, CMatch *match) {
	...
	commands.emplace_back("join", "Joins a match", " [match_id]", [&] (Tokenizer& cmd) {
		...
		CMatchManager::Instance()->HLJoinMatch(matchId, MakeMatchResultHandler([&] (eErrorCode err, const CCloudResult *result, CMatch *match) {
	...
	commands.emplace_back("finish", "Finishes a match", " match_id", [&] (Tokenizer& cmd) {
		...
		CMatchManager::Instance()->HLRestoreMatch(matchId, MakeMatchResultHandler([&] (eErrorCode err, const CCloudResult *result, CMatch *match) {
			...
			if (match) {
				match->Finish(true, MakeMatchResultHandler([&] (eErrorCode err, const CCloudResult *result, CMatch *match) {
	...
	commands.emplace_back("create", "Creates a match", " [description] [properties]", [&] (Tokenizer& cmd) {
		...
		if (t == Tokenizer::OpeningBrace || t == Tokenizer::OpeningBracket) {
			properties = CHJSON::parse(arg);
		} else {
			properties = new CHJSON;
		}
		// Also put a seed for random gameplay (would probably better be placed in globalState)
		properties->Put("seed", (int) time(NULL));
		config.Put("customProperties", properties);
...
void Lobby::StartGame(CMatch *match) {
	printf("Starting match %s\n", match->GetMatchId());
	resultMatch = mkref(match);
	running = false;
}
